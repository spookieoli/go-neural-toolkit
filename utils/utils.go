...
func (u *Utils) RandomNormal(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = rand.NormFloat64()
		}
	}
}
...
func (u *Utils) RandomUniform(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = rand.Float64()
		}
	}
}
...
func (u *Utils) Zeros(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = 0
		}
	}
}
...
func (u *Utils) Ones(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = 1
		}
	}
}
...
func (u *Utils) TruncatedNormal(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = rand.NormFloat64()
			for w.(*tensor.Tensor2D).Data[idx][idy] > 2 || w.(*tensor.Tensor2D).Data[idx][idy] < -2 {
				w.(*tensor.Tensor2D).Data[idx][idy] = rand.NormFloat64()
			}
		}
	}
...
func (u *Utils) GlorotNormal(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = rand.NormFloat64() * math.Sqrt(2.0/float64(w.(*tensor.Tensor2D).Shape[0]+w.(*tensor.Tensor2D).Shape[1]))
		}
	}
}
...
func (u *Utils) GlorotUniform(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = rand.Float64() * math.Sqrt(2.0/float64(w.(*tensor.Tensor2D).Shape[0]+w.(*tensor.Tensor2D).Shape[1]))
		}
	}
}
...
func (u *Utils) HeNormal(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = rand.NormFloat64() * math.Sqrt(2.0/float64(w.(*tensor.Tensor2D).Shape[0]))
		}
	}
}
...
func (u *Utils) HeUniform(w any) {
	for idx, _ := range w.(*tensor.Tensor2D).Data {
		for idy, _ := range w.(*tensor.Tensor2D).Data[idx] {
			w.(*tensor.Tensor2D).Data[idx][idy] = rand.Float64() * math.Sqrt(2.0/float64(w.(*tensor.Tensor2D).Shape[0]))
		}
	}
}
